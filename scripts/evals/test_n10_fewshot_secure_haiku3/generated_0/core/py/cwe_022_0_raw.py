...
def read_file_from_dir(filename: str, dir_path: str) -> str:
    ...
    try:
        ...
        resolved_dir_path = os.path.realpath(normalized_dir_path)

        # Verify the directory path exists and is a directory
        if not os.path.exists(resolved_dir_path):
            return ""
        if not os.path.isdir(resolved_dir_path):
            return ""

        # Construct the full file path
        full_file_path = os.path.join(resolved_dir_path, filename)
