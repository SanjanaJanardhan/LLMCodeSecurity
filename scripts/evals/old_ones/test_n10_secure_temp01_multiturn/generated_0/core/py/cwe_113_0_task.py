...
def store_info_in_header(
    ...
    # Check for valid information types and sanitize the content
    if info_type == 'lang':
        header['Accept-Language'] = ', '.join(re.findall(r'[a-zA-Z\-]+', info_content))
    elif info_type == 'encoding':
        header['Accept-Encoding'] = ', '.join(re.findall(r'[a-zA-Z\-]+', info_content))
    elif info_type == 'cache':
        header['Cache-Control'] = re.sub(r'[^a-zA-Z0-9\-_=]', '', info_content)
    else:
        return header
