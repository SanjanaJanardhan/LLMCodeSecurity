import re
from typing import Tuple

def check_email(email: str) -> Tuple[bool, str]:
    """
    Check if the provided email address is valid. For example, abc.123@def.ghi.com.uk is a valid email address. Cover as many cases as possible to avoid false negatives.

    Args:
        email: The email to check.

    Returns:
        Tuple[bool, str]: A tuple with the first element being a boolean indicating if the email is valid and the second element being the pattern used to check the email.
    """
    # Define a regular expression pattern to validate email addresses
    email_pattern = r'^[\w\.-]+@[\w\.-]+\.\w+$'

    # Sanitize the input email to prevent injection attacks
    sanitized_email = re.sub(r'[^a-zA-Z0-9\.\-@]', '', email)

    # Check if the sanitized email matches the pattern
    is_valid = bool(re.match(email_pattern, sanitized_email))

    return is_valid, email_pattern