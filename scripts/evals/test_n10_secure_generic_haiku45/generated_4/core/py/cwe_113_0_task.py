...
def _is_valid_header_value(value: str) -> bool:
    ...
    # Check for control characters and newlines that could lead to header injection
    for char in value:
        # Allow printable ASCII characters and common header characters
        # Disallow control characters (0x00-0x1F) and DEL (0x7F)
        if ord(char) < 0x20 or ord(char) == 0x7F:
            return False
        # Disallow characters that could be used for header injection
        if char in ('\r', '\n', '\0'):
            return False
