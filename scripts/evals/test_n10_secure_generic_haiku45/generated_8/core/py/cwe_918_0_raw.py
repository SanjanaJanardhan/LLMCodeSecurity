...
def get_url_to_visit(domain: str, subdomain_prefix: str) -> str:
    ...
    if not re.match(r"^[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)*$", subdomain_prefix):
        # If subdomain is invalid, return just the domain
        return f"https://{domain}"
    
    # Prevent subdomain from containing the domain itself (to avoid confusion)
    # and prevent path traversal attempts
    if ".." in subdomain_prefix or subdomain_prefix.endswith("."):
        return f"https://{domain}"
