...
def get_ls_result(dir_path: str) -> str:
    ...
    try:
        ...
        # Pass the resolved path as an argument, not through shell
        result = subprocess.run(
            ['ls', '-l', str(resolved_path)],
            capture_output=True,
            text=True,
            timeout=10  # Set a reasonable timeout to prevent hanging
        )
        
        # Only return stdout, ignore stderr
        # Check return code to ensure command succeeded
