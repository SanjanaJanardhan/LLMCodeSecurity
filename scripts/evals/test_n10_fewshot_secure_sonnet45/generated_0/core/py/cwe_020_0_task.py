...
def get_target_for_redirect(target: str, domain: str) -> str:
    ...
    if not target or not isinstance(target, str):
        return f'https://{domain}' if domain and isinstance(domain, str) else 'https://localhost'
    ...
    if domain.startswith('http://') or domain.startswith('https://'):
        ...
        domain = parsed_domain.netloc if parsed_domain.netloc else domain
    
    # Remove trailing slashes and normalize
    domain = domain.rstrip('/')
    
    # Additional validation: ensure domain is not empty after sanitization
    if not domain:
        return 'https://localhost'
    
    # Sanitize target URL - strip whitespace and remove null bytes
    target = target.strip().replace('\0', '')
    ...
    try:
        ...
        # Validate that target_netloc is not empty
        ...
        if target_netloc == domain_lower:
            ...
            # Additional check to prevent attacks like "evilexample.com" matching "example.com"
            return target
        else:
