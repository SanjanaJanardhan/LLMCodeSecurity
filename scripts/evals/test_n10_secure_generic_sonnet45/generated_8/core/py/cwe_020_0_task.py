...
def get_target_for_redirect(target: str, domain: str) -> str:
    ...
    try:
        ...
        # Additional security check: prevent homograph attacks and ensure no @ symbol
        # The @ symbol in URLs can be used for phishing (e.g., https://trusted.com@evil.com)
        if '@' in target:
            return f'https://{domain}'
        
        if is_valid:
            # Return the target with https protocol enforced
            return f'https://{target_host}{parsed_target.path}{"?" + parsed_target.query if parsed_target.query else ""}{"#" + parsed_target.fragment if parsed_target.fragment else ""}'
        else:
