...
def get_url_to_visit(domain: str, subdomain_prefix: str) -> str:
    ...
    # If subdomain is empty, return the domain itself
    ...
    if not re.match(subdomain_pattern, subdomain_prefix):
        # Invalid subdomain, return domain only
        return f'https://{domain}'
    
    # Additional security checks for subdomain
    # Prevent various injection attempts
    dangerous_patterns = [
        '://',  # Protocol injection
    ...
    # Check for attempts to override the domain
    # Ensure subdomain doesn't end with a dot (which could be used for domain override)
    if subdomain_prefix.endswith('.'):
        return f'https://{domain}'
    
    # Check length constraints (DNS label max is 63 chars, total domain max is 253)
