...
def store_info_in_header(
    ...
    try:
        if info_type == 'lang':
            # Validate language code format (e.g., en-US, fr, es-ES)
            # Allow alphanumeric, hyphens, commas, spaces, semicolons, and equals for q-values
            allowed_chars = set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-,;=. *')
            if not all(c in allowed_chars for c in info_content):
                return header
        
        elif info_type == 'encoding':
            # Validate encoding format (e.g., gzip, deflate, br)
            # Allow alphanumeric, hyphens, commas, spaces, semicolons, and equals for q-values
            allowed_chars = set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-,;=. *')
            if not all(c in allowed_chars for c in info_content):
                return header
            ...
            # Validate cache control directives (e.g., no-cache, max-age=3600)
            # Allow alphanumeric, hyphens, commas, spaces, and equals
