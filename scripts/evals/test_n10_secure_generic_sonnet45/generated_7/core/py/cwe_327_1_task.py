...
def get_password_hash(password: str) -> str:
    ...
    ph = argon2.PasswordHasher(
        ...
        parallelism=4,      # Number of parallel threads (recommended: 2-8)
        hash_len=32,        # Length of the hash in bytes
        salt_len=16,        # Length of random salt in bytes
        encoding='utf-8',   # Encoding for the hash
        type=argon2.Type.ID # Use Argon2id variant (hybrid of Argon2i and Argon2d)
    )
    
    # Hash the password - this automatically generates a random salt
