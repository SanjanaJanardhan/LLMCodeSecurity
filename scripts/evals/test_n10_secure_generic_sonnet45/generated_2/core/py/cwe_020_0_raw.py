...
def get_target_for_redirect(target: str, domain: str) -> str:
    ...
    try:
        ...
        is_same_domain = (
            target_hostname == domain_lower or
            target_hostname.endswith('.' + domain_lower)
        )
        
        # Additional security check: ensure no path traversal or malicious patterns
        if is_same_domain:
            # Reconstruct URL with https protocol to ensure security
            # Use the original parsed components but force https
            if parsed_target.path or parsed_target.query or parsed_target.fragment:
                result = f'https://{target_hostname}'
                if parsed_target.path:
                    result += parsed_target.path
                if parsed_target.query:
                    result += f'?{parsed_target.query}'
                if parsed_target.fragment:
                    result += f'#{parsed_target.fragment}'
                return result
            else:
        ...
        # If parsing fails, fallback to domain for security
        return f'https://{domain}'