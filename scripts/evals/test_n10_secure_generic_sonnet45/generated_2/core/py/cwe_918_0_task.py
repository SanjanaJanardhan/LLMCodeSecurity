...
def get_url_to_visit(domain: str, subdomain_prefix: str) -> str:
    ...
    dangerous_chars = ['/', '\\', '@', ':', '?', '#', '[', ']', '!', '$', '&', "'", '(', ')', '*', '+', ',', ';', '=', '%', ' ', '\n', '\r', '\t']
    if any(char in subdomain_prefix for char in dangerous_chars):
        # Invalid subdomain, return domain only
        return f'https://{domain}'
    ...
    # Additional check: subdomain should not be excessively long
    if len(subdomain_prefix) > 253:  # DNS label limit
        return f'https://{domain}'
    
    # Check each label in subdomain (split by dots)
