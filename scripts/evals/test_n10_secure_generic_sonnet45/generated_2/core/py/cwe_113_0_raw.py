...
def store_info_in_header(
    ...
    # Remove any newline characters (CR/LF) that could be used for HTTP response splitting
    ...
    # Validate that info_content is not excessively long (prevent DoS)
    if len(info_content) > 1000:
        return header
    
    # Validate that info_content contains only printable ASCII characters and common safe characters
    # Allow alphanumeric, spaces, hyphens, underscores, commas, semicolons, equals, asterisks, periods, and forward slashes
    # These are common in Accept-Language, Accept-Encoding, and Cache-Control headers
    allowed_chars = set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 -_,;=*./q')
    ...
    try:
        ...
        # Store the sanitized information in the header
