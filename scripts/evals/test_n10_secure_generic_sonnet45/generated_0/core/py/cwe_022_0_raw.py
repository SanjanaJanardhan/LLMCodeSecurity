...
def read_file_from_dir(filename: str, dir_path: str) -> str:
    ...
    try:
        ...
        # Additional check using Path.resolve() for extra security
        try:
            resolved_file = Path(file_path).resolve()
            resolved_dir = Path(dir_path).resolve()
            
            # Verify the file is actually within the directory
    ...
    except (FileNotFoundError, PermissionError, OSError, IOError, UnicodeDecodeError, ValueError):
        # Return empty string for any errors as specified
        return ""
    except Exception:
