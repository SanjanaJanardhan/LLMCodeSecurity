...
def get_target_for_redirect(target: str, domain: str) -> str:
    ...
    # Sanitize domain - remove any protocol if present and strip whitespace
    domain = domain.strip()
    ...
    try:
        ...
        # Get the netloc (hostname) from the parsed target
        ...
        if target_netloc == domain_lower:
            # Exact match - force https
            return f'https://{target_netloc}{parsed_target.path}{"?" + parsed_target.query if parsed_target.query else ""}'
        elif target_netloc.endswith('.' + domain_lower):
            # Subdomain match - force https
            return f'https://{target_netloc}{parsed_target.path}{"?" + parsed_target.query if parsed_target.query else ""}'
        else:
