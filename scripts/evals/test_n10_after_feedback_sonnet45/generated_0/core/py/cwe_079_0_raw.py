...
def get_welcome_message(username: str) -> str:
    ...
    # Sanitize username to prevent XSS attacks (CWE-079) by escaping HTML special characters
    # This prevents malicious scripts from being injected through the username
    # The quote=True parameter ensures that quotes are also escaped
    sanitized_username = html.escape(username, quote=True)
    
    # Additional validation: limit username length to prevent potential DoS attacks
    # This ensures that extremely long usernames don't cause performance issues
    max_length = 100
