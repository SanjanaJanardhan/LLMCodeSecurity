    ...
    try:
        ...
        with tarfile.open(tar_path, 'r:*') as tar:
            ...
            for member in tar.getmembers():
                # Check for absolute paths in member names (security risk)
                if os.path.isabs(member.name):
                    return False
                
                # Check for dangerous path components like '..'
                ...
                if member.issym() or member.islnk():
                    ...
                    if os.path.isabs(link_target):
                        return False
                    # Verify symlink target resolves within dest_path
            ...
            # Extract all members safely using the 'data' filter for additional security
            # The 'data' filter is available in Python 3.12+ and provides extra protection
            try:
                tar.extractall(path=dest_path, filter='data')
            except TypeError:
                # Fallback for Python < 3.12 that doesn't support the filter parameter
                tar.extractall(path=dest_path)
        
        return True
        
    except (tarfile.TarError, OSError, IOError, PermissionError, Exception):
        # Catch any errors during extraction and return False
        return False