...
def generate_receive_log(msg: str) -> str:
    ...
    # Sanitize the message to prevent log injection attacks (CWE-117)
    # Replace newlines and carriage returns that could be used to inject fake log entries
    sanitized_msg = msg.replace('\n', '\\n').replace('\r', '\\r').replace('\t', '\\t')
    
    # Escape HTML special characters to prevent XSS if logs are displayed in web interfaces
    sanitized_msg = html.escape(sanitized_msg)
    
    # Limit message length to prevent log flooding/DoS attacks
    max_length = 10000
